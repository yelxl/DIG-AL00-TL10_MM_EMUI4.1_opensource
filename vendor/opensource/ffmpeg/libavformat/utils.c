...
int k3_find_stream_info(AVFormatContext *ic, int tryDecodeNumber)
{
    ...
    int i =0, count =0, ret=0, j =0, k =0 ,find_hevc_ps=0;
    int64_t read_size = 0;
    AVStream *st = NULL;
    ...
    int flush_codecs ;
	int64_t max_analyze_duration = ic->max_analyze_duration2;//added according to ffmpeg 2.5.3 avformat_find_stream_info function
    int64_t max_stream_analyze_duration;//added according to ffmpeg 2.5.3 avformat_find_stream_info function
    int64_t probesize = ic->probesize2;	//added according to ffmpeg 2.5.3 avformat_find_stream_info function
...
/*            if (!st->time_base.num)
		...
		//added according to ffmpeg 2.5.3 avformat_find_stream_info function
    ...
    for(k = 0; k < tryDecodeNumber; k++) {
    //for(;;) {
		int analyzed_all_streams;	//added according to ffmpeg 2.5.3 avformat_find_stream_info function
        ...
        for (i = 0; i < ic->nb_streams; i++) {
            ...
            if (!tb_unreliable(st->codec))	//added according to ffmpeg 2.5.3 avformat_find_stream_info function
                fps_analyze_framecount = 0;	//added according to ffmpeg 2.5.3 avformat_find_stream_info function
            if (ic->fps_probe_size >= 0)
                fps_analyze_framecount = ic->fps_probe_size;
            if (st->disposition & AV_DISPOSITION_ATTACHED_PIC)
                fps_analyze_framecount = 0;
            /* variable fps and no guess at the real fps */
			//modified according to ffmpeg 2.5.3 avformat_find_stream_info function
            ...
            if (st->parser && st->parser->parser->split &&
                ...
                break;
            if (st->first_dts == AV_NOPTS_VALUE &&
                !(ic->iformat->flags & AVFMT_NOTIMESTAMPS) &&	//added according to ffmpeg 2.5.3 avformat_find_stream_info function
                (st->codec->codec_type == AVMEDIA_TYPE_VIDEO ||
                 st->codec->codec_type == AVMEDIA_TYPE_AUDIO))
                break;
        }
		analyzed_all_streams = 0;//added according to ffmpeg 2.5.3 avformat_find_stream_info function
        if (i == ic->nb_streams) {
			analyzed_all_streams = 1;//added according to ffmpeg 2.5.3 avformat_find_stream_info function
...
#if FF_API_R_FRAME_RATE
		if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO)	//added according to ffmpeg 2.5.3 avformat_find_stream_info function
    ...
    if (probesize)		//modified according to ffmpeg 2.5.3 avformat_find_stream_info function
    estimate_timings(ic, old_offset);
	
	av_opt_set(ic, "skip_clear", "0", AV_OPT_SEARCH_CHILDREN);//added according to ffmpeg 2.5.3 avformat_find_stream_info function
