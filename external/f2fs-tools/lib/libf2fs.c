...
int f2fs_get_device_info(struct f2fs_configuration *c)
{
	...
	long long int wanted_total_sectors = c->total_sectors;
	...
	if(wanted_total_sectors < 0 && wanted_total_sectors + c->total_sectors > 0) {
		MSG(0, "Info: total device sectors = %"PRIu64" (in %u bytes)\n",
				c->total_sectors, c->sector_size);
		c->total_sectors += wanted_total_sectors;
	}
	if (wanted_total_sectors > 0 && wanted_total_sectors < c->total_sectors) {
		MSG(0, "Info: total device sectors = %"PRIu64" (in %u bytes)\n",
					c->total_sectors, c->sector_size);
		c->total_sectors = wanted_total_sectors;

		if(c->extent_sectors < c->total_sectors)
			c->extent_sectors = 0;
	}
